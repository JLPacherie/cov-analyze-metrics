...
# The configuration dir (with the enabledCheckers configs)
...
function showHelp() {
	echo \
"
 This script allows you to create defects from enabledCheckers based on code metrics
 ...
 Those enabledCheckers require that the code base had already been analyzed so that the
 ...
 --enable-default               Enable all metric enabledCheckers
...
# To retrieve some stats from the IDIR
...
if [[ $nbCheckers -lt 1 ]]; then
	echo " - There's no enabledCheckers defined, use option --enable-checker CHECKER_NAME=threshold"
	echo " - Available metrics enabledCheckers: "
	...
	echo ""
	echo " - There are $nbCheckers enabledCheckers defined."
...
while  IFS='' read -r strLine || [[ -n "$strLine" ]];  do
  ...
  #
  # Process extracted metrics with each of the enabled enabledCheckers
  #
