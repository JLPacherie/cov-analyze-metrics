...
public class Checker {
	...
	protected List<Metrics> _allMetrics = new ArrayList<Metrics>();
	 ...
	 */
	public boolean filter(Measurable measured) {
	...
	public Defect check(Measurable measured) {
		...
		assert _allMetrics.size() > 0 : "There's no metrics to check for in this checker ??";
		...
		List<String> violatingMetrics = _allMetrics.stream()
				...
				.map(metric -> measured.getMetric(metric.metric) > metric.value ? metric.metric : null)//
